...
#nullable disable
    public class Some<T> : IOption<T>, IEquatable<T>, IEquatable<Some<T>>, IEquatable<IOption<T>>
    {
        ...
        public static bool operator == (Some<T> x, Some<T> y) => x.value.Equals(y.value);
        public static bool operator == (Some<T> x, IOption<T> y) => y.HasValue(out T val) && x.value.Equals(val);
        ...
        public static bool operator != (Some<T> x, Some<T> y) => !(x == y);
