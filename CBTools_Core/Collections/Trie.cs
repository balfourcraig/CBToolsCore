...
#nullable disable
    public class Trie<T> : IEnumerable<Trie<T>> 
    {
        ...
        {
            Queue<Trie<T>> q = new Queue<Trie<T>>();
            HashSet<Trie<T>> discovered = new HashSet<Trie<T>>();
            ...
            if (t != null)
            {
                ...
                while (t.children.Count == 0 && !t.completesString)
                {
                    if(t.letter.HasValue)
                        t.parent.children.Remove(t.letter.Value);
                    t = t.parent;
                }
            }
        }
        ...
        public int MaxDepth(ReadOnlySpan<char> prefix)
        {
            ...
            {
                if (this.children.TryGetValue(caseSensitive ? prefix[0] : char.ToUpperInvariant(prefix[0]), out Trie<T> set))
        ...
        public Trie<T> TraverseTo(ReadOnlySpan<char> prefix)
        {
            ...
            else if (children.TryGetValue(caseSensitive ? prefix[0] : char.ToUpperInvariant(prefix[0]), out Trie<T> n))
        ...
        public bool Contains(ReadOnlySpan<char> prefix)
        {
            if (prefix.Length == 0)
                ...
                return false;
            else if (children.TryGetValue(caseSensitive ? prefix[0] : char.ToUpperInvariant(prefix[0]), out Trie<T> next))
